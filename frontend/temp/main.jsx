...
function LandingPageV2({
  ...
  const authStatusLabel = authError
    ...
    : authLoading
      ? 'Sincronizando'
      : 'Configuracion lista';
  const authStatusDetail = authError
    ? authError
    : authLoading
      ? 'Esperando respuesta de la API remota.'
      : `${variableCount} variables cargadas desde la API.`;
    ...
    <main className="auth-shell auth-landing">
      ...
      <div className="landing-shell">
        <header className="landing-topbar">
          <div className="brand-lockup auth-brand-lockup landing-brand-lockup">
            <img src={BRAND_ICON} alt="" aria-hidden="true" />
            <div>
              <p className="eyebrow">{projectName}</p>
              <span>Acceso seguro a tu ecosistema de archivos</span>
          ...
          <div className="landing-topbar-actions">
            <span className={`landing-status-chip ${authError ? 'danger' : authLoading ? 'loading' : 'ready'}`}>
              {authError ? <LockKeyhole size={14} /> : <DatabaseZap size={14} />}
              {authStatusLabel}
            </span>

            <button type="button" className="secondary" onClick={hasSession ? onGoDashboard : onGoLogin}>
              {hasSession ? 'Ir al dashboard' : 'Iniciar sesion'}
              <MoveRight size={16} />
            </button>
          </div>
        </header>
        ...
        <section className="landing-hero">
          <div className="landing-hero-copy">
            ...
            <h1>Una entrada premium para proyectos, archivos y autenticacion empresarial.</h1>

            <p className="landing-lead">
              La pantalla carga la configuracion remota, arma las URLs de login y registro, y deja
              listo el flujo de callback para validar la sesion antes de entrar al dashboard.
            </p>
            ...
            <div className="landing-metrics">
              <LandingMetric
                icon={DatabaseZap}
                label="Variables remotas"
                value={authLoading ? '...' : String(variableCount)}
              ...
              <LandingMetric
                ...
                label="Redirect URI"
                value={redirectUri}
                detail="El callback queda listo para intercambiar el code."
              />
              <LandingMetric
                icon={Building2}
                label="Sesion / tenant"
                value={hasSession ? getTenantDisplayName(session) : 'Sin sesion'}
                detail={hasSession ? getSessionDisplayName(session) : 'Aun no se ha guardado una sesion.'}
              />
            </div>
          </div>
          ...
          <div className="landing-visual">
            <article className="landing-preview-card">
              <div className="landing-preview-head">
                ...
                <h2>{authLoading ? 'Sincronizando autenticacion' : 'Secuencia lista para produccion'}</h2>
                <p>{description}</p>
              </div>
              ...
              <div className="landing-flow">
                <LandingStep
                  ...
                  description="Se arma la URL con app_id, redirect_uri y api_key desde la configuracion remota."
                />
                <LandingStep
                  index="02"
                  icon={MoveRight}
                  title="Recibir callback"
                  description="El proveedor responde con code y state authenticated para continuar el flujo."
                />
                <LandingStep
                  index="03"
                  icon={BadgeCheck}
                  title="Validar sesion"
                  description="Intercambiamos el code por tokens y validamos el access token antes de persistir la sesion."
                />
              </div>
              ...
              <div className="landing-signal-grid">
                <LandingSignal
                  icon={DatabaseZap}
                  label="Auth URL"
                  value={authUrl || 'Esperando config'}
                  detail="Base remota para login y registro."
                />
                <LandingSignal
                  icon={PanelLeft}
                  label="Callback"
                  value={redirectUri}
                  detail="Ruta local preparada para el intercambio."
                />
                <LandingSignal
                  icon={LockKeyhole}
                  label="Estado"
                  value={authError ? 'Con error' : authLoading ? 'Cargando' : 'Listo'}
                  detail={authError || 'La autenticacion puede abrir el dashboard.'}
                />
              </div>
            </article>
            ...
            <div className="landing-side-stack">
              <article className="landing-mini-panel">
                <p className="section-kicker">Configuracion viva</p>
                <strong>{projectName}</strong>
                <span>{authStatusDetail}</span>
                <div className="landing-mini-rows">
                  <AuthStateRow label="App ID" value={appId} />
                  <AuthStateRow label="Redirect" value={redirectUri} />
                  <AuthStateRow label="Backend" value={authUrl || 'Sin URL'} />
                </div>
              </article>

              <article className="landing-mini-panel accent">
                <p className="section-kicker">Sesion tenant</p>
                <strong>{hasSession ? getSessionDisplayName(session) : 'Sin sesion activa'}</strong>
                <span>
                  {hasSession
                    ? getTenantDisplayName(session)
                    : 'Inicia sesion para normalizar la identidad y volver al dashboard.'}
                </span>

                <div className="landing-mini-actions">
                  <button type="button" onClick={onGoLogin}>
                    <LogIn size={16} />
                    Iniciar sesion
                  </button>
        ...
        <section className="landing-highlights">
          <LandingHighlight
            ...
            description="La sesion solo se guarda despues de intercambiar el code y verificar el access token."
          />
          <LandingHighlight
            icon={Layers3}
            title="Organizacion por capas"
            description="El producto ya esta preparado para proyectos, carpetas y archivos por contexto de tenant."
          />
          <LandingHighlight
            icon={Workflow}
            title="Flujo consistente"
            description="Login, callback y dashboard comparten la misma configuracion remota y el mismo estado de sesion."
          />
        </section>
        ...
        <footer className="landing-footer">
          ...
          <span>{authError ? authError : authLoading ? 'Cargando configuracion de autenticacion...' : description}</span>
        </footer>
      </div>
    </main>
  );
